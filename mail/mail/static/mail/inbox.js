...
function compose_email() {
  ...
  // Submit button disabled if compose fields are empty
...
function load_mailbox(mailbox) {
  ...
  // Show the mailbox name, read/unread attributes
  ...
  .then(emails => {
    emails.forEach(item => {
      ...
      if (item.read===true) {
        ...
        element.setAttribute('class', "NotRead")
      }
  ...
  .then(email => {
    ...
    if (!(mailbox==="sent")){
      ...
      if (email.archived===false){
        element.setAttribute('class', "btn btn-sm btn-outline-secondary");
        element.innerHTML = "Archive";
        element.addEventListener('click', () => archive(`${email.id}`));
      }
      else{
        element.setAttribute('class', "btn btn-sm btn-outline-dark");
    ...
    newelement.setAttribute('class', "btn btn-sm btn-outline-warning");
